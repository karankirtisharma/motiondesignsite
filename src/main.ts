...
$('lightweight-button').onclick=()=>setLightweight(!lightweight);$('settings-lightweight').onclick=()=>{dialog.close();setLightweight(true);};$('retry-button').onclick=()=>location.reload();
...
async function start3D(){if(starting)return;starting=true;$('loading-status').hidden=false;$('loading-copy').textContent='Preparing the building';$('retry-button').hidden=true;
 try{const {Experience}=await import('./runtime/experience');runtime=new Experience($<HTMLCanvasElement>('scene'),{quality,reduced,onReady:()=>{document.body.dataset.readyMs=performance.now().toFixed(0);if(!lightweight)document.body.classList.add('gpu-ready');$('loading-status').hidden=true;},onStatus:text=>{$('loading-copy').textContent=text;$('loading-status').hidden=!text;},onChange:s=>{
  if(lightweight)return;$('progress-fill').style.transform=`scaleY(${s.phase})`;
  const ended=s.phase>=.998&&!s.exploring;
  const state=[s.room,s.exploring,s.reception,s.anchor,s.chapterIndex,s.description,ended].join('|');if(state===lastSceneState)return;lastSceneState=state;
  document.body.classList.toggle('interior',s.room!=='exterior');document.body.classList.toggle('exploring',s.exploring);
  document.body.classList.toggle('journey-ended',ended);$('journey-finale').setAttribute('aria-hidden',String(!ended));
